...
def map_start(host, port, split):
    '''Fonction pour envoyer le split à chaque host'''
...
def shuffle_start(host, port):
    '''Fonction pour envoyer shuffle start à chaque host'''
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def reduce_start(host, port):
    '''Fonction pour envoyer reduce start à chaque host'''
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def bye(host, port):
    '''Fonction pour envoyer bye à chaque host'''
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def main():
    ...
    with open('machines.txt', 'r') as f:
        ...
        for i in range(len(hosts)):
            ...
            splits.append(
                f"/tmp/msauveur-23/adeployer/splits/grosfichier6.warc.wet.part1.part{i+1}")
